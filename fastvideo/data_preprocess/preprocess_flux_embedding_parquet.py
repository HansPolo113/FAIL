...
def suppress_pipeline_warnings():
    """Suppress specific pipeline warnings"""
    import logging
    import warnings

    # Suppress CLIP/tokenizer truncation warnings from diffusers
...
class ParquetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.vae_debug:
            latents = torch.load(
                os.path.join(
                    args.output_dir, "latent", item["latent_path"]
                ),
                map_location="cpu",
            )
        else:
...
def main(args):
    ...
    train_dataset = ParquetDataset(args.parquet_path, args.vae_debug)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--parquet_path", type=str, default="./data/train_25k.parquet")
